...
class TenpayRGeneraterPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        project.plugins.all {
            when (it) {
                is FeaturePlugin -> {
                    project.extensions[FeatureExtension::class].run {
                        configTR(project, featureVariants)
                        configTR(project, libraryVariants)
                    }
                }
                is LibraryPlugin -> {
                ...
                is AppPlugin -> {
                    project.extensions[AppExtension::class].run {
                        configTR(project, applicationVariants)
                    }
                }
            }
        }
    ...
    private fun configTR(project: Project, variants: DomainObjectSet<out BaseVariant>) {
        variants.all { variant ->
            val outputDir = project.buildDir.resolve(
                "generated/source/tr/${variant.dirName}"
            )
                    ...
                    val rFile =
                        project.files(
                            when (processResources) {
                                ...
                                else -> throw IllegalArgumentException("Minimum supported Android Gradle Plugin is 3.3.0")
                            }
                        )
                            .builtBy(processResources)
                    val generate = project.tasks.create(
                        "generate${variant.name.capitalize()}TR",
                        TRGenerator::class.java
                    ) {
